...
public class Log extends BaseTest {
	...
	public static String getTestRunMachineInfo(WebDriver driver) {
	...
	public static ExtentTest testCaseInfo(String description, String testName) {
	 ...
	 * case status linked to respective screenshot (level=info) depends on system
	 * variable contentAutoStudThread status, if true it will take the student
	 * details else message print test case description
	 * 
	 * @param driver
	 * @param description
	 ...
	 * the student details else message print custom test case status message with
	 * screenshot
	 * 
	 * @param description
	...
	public static void event(String description) {
		...
		if (configProperty.hasProperty("contentAutoStudThread")
			...
			if (null == studentsLogs.get(tName)) {
				...
				studentLogs.add(
						EVENT_HTML_BEGIN + "--- " + tName.split("%")[1] + " execution started---" + EVENT_HTML_END);
			} else {
	 ...
	 * particular action (level=debug) depends on system variable
	 * contentAutoStudThread status, if true it will take the student details else
	 * event print the page object custom message
	 * 
	 * @param description
	 *            test case
	 * @param duration
	...
	/**
	 * pass print the test case custom message in the log with screenshot
	 * (level=info) depends on system variable contentAutoStudThread status, if true
	 * it will take the student details else message print the test case custom
	 * message in the log with screenshot
	 * 
	 ...
	 *            to take screenshot
	 */
	public static void fail(String description, WebDriver driver, Boolean... takeScreenShot) {
		String inputFile = takeScreenShot(driver);
		String tName = Thread.currentThread().getName();
		...
		if (configProperty.hasProperty("contentAutoStudThread")
			...
			if (finalDecision) {
				// String inputFileFail = takeScreenShot(driver);
				studentsLogs.get(tName).add(MESSAGE_HTML_BEGIN + description + "&emsp;"
						+ getScreenShotHyperLink(inputFile) + MESSAGE_HTML_END);
			} else {
				studentsLogs.get(tName).add(MESSAGE_HTML_BEGIN + description + MESSAGE_HTML_END);
	...
	public static void exception(Exception e, WebDriver driver) throws Exception {
		...
		if (e instanceof SkipException) {
			if (configProperty.hasProperty("contentAutoStudThread")
					&& tName.contains(configProperty.getProperty("contentAutoStudThread"))) {
				studentsLogs.get(tName)
						.add(SKIP_EXCEPTION_HTML_BEGIN + eMessage + SKIP_HTML_END1 + screenShotLink + SKIP_HTML_END2);
			} else {
			...
			if (configProperty.hasProperty("contentAutoStudThread")
					&& tName.contains(configProperty.getProperty("contentAutoStudThread"))) {
				studentsLogs.get(tName)
						.add(FAIL_HTML_BEGIN + eMessage + FAIL_HTML_END1 + screenShotLink + FAIL_HTML_END2);
	...
	public static void addSauceJobUrlToReport(WebDriver driver, String sauceLink) {
		...
		if (tName != null && !tName.contains("Student")) {
			...
			params[0] = (params[0] == null || !params[0].toString().contains("SauceLab link"))
					...
					: params[0] + "<br>SauceLab link: <a href='" + sauceLink + "' target='_blank'>" + sauceLink
							+ "</a>";
			Reporter.getCurrentTestResult().setParameters(params);
			ExtentReporter
					.addAttribute("SauceLab link: <a href='" + sauceLink + "' target='_blank'>" + sauceLink + "</a>");
		} else {
			studentsLogs.get(tName).add(MESSAGE_HTML_BEGIN + "SauceLink for student[" + tName.split("%")[1] + "] is : "
					+ "<a href='" + sauceLink + "' target='_blank'>" + sauceLink + "</a>" + MESSAGE_HTML_END);
		}
	}
	...
	public static String getScreenshots() {
		String tName = Thread.currentThread().getName();
		return screenShotFolderPath + failedImageUrl.get(tName);
	}
