...
public class ExtentReporter {
	...
	private static ExtentTest startTest(String description) {
		...
		String caseId = iTestResult != null ?iTestResult.getName() :" ";
		...
		if (tests.containsKey(hashCode)) {
			...
			if (iTestResult == null || !iTestResult.getMethod().isTest()) {
				test = new ExtentTest(caseId +" - " + description, testName);
			} else {
				test = getReportInstance(iTestResult).startTest(caseId +" - "+ description, testName + iTestResult.getMethod())
	...
	private static ExtentTest getTest() {
		return startTest(globalVariables.testCaseDescription);
	}
	...
	public static void closeReport(List<ITestResult> allTestCaseResults, String outdir) {
		...
		if (report != null) {
			...
			String env = globalVariables.environmentName;
			
			File reportFolder = new File("test-reports/TestReports");
			File reportSourceFile = new File("target/surefire-reports/ExtentReport.html");
