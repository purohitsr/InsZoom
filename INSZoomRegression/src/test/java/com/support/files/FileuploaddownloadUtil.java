...
public class FileuploaddownloadUtil extends TestCase {
...
return file_content;
		 ...
		 public static ArrayList get_downloaded_files(RemoteWebDriver remoteDriver) {
			 ArrayList filesFound = null;
				try {
					...
					Object obj =remoteDriver.executeScript("return document.querySelector('downloads-manager').shadowRoot.querySelector('#downloadsList').items.filter(e => e.state === 'COMPLETE').map(e => e.filePath || e.file_path || e.fileUrl || e.file_url);");
				} catch (Exception e) {
					System.err.println(e);
				}
				return filesFound;
